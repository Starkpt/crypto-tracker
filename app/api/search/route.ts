...
export async function GET(request: NextRequest) {
  ...
  const paramsForCoinsSearch: Record<string, string | number> = {};

  // Iterate through each parameter in the searchParams
  searchParams.forEach((value: string | number, key: string) => {
    // Include only the 'query' parameter in paramsForCoinsSearch if it exists
    if (key === "query" && value) paramsForCoinsSearch[key] = value;
  ...
  const URLCoinsSearchParams = new URLSearchParams(paramsForCoinsSearch);
